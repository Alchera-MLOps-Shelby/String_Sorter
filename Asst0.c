...
// Struct to start Making Linked List for sorting
struct Node
{
 char* token;
 struct Node* next;
...
// To append character by charcter until a token is hit & then terminate to a string
...
int main(int argc, char* argv[]) // argc = # args on command line //argv[] values at locations 
{
 ...
 for(index = 1; index < argc; index++) // Iterate through each element on command line
 {
   ...
   while(iterateStr < length) // Iterate through the sentence & concatenate each letter until a token is found, then sort it. If no Token found then sort & move on to next sentence at top of for loop
   {
    ...
    if(tokenFound == 0)// Is a token
    {
      ...
      //TODO: sortLinkedList(word); //Write method to pass a parsed word into a data structure & sort it
